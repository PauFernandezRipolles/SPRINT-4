...
SELECT card_id,
    (SELECT declined FROM transactions as t1 WHERE t1.card_id = transactions.card_id 
    ORDER BY timestamp DESC LIMIT 1) as trans_ultima,
    (SELECT declined FROM transactions as t2 WHERE t2.card_id = transactions.card_id 
    ORDER BY timestamp DESC LIMIT 1 OFFSET 1) as trans_penultima,
    (SELECT declined FROM transactions as t3 WHERE t3.card_id = transactions.card_id 
    ORDER BY timestamp DESC LIMIT 1 OFFSET 2) as trans_avantpenultima
FROM (SELECT DISTINCT card_id FROM transactions) as transactions;
